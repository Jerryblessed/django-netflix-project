...
def like_movie(request, movie_id):
    ...
    if not created:
        like.is_liked = not like.is_liked  # Toggle the like status
        like.save()
    else:
        like.is_liked = True

    messages.success(request, "Movie like status updated.")
    return redirect('movie_list')  # Adjust to your movie listing view
...
def index_view(request):
    ...
    most_liked_movie = Movie.objects.filter(resolution__in=resolutions).order_by('-like_count').first()
...
def watch_movie_view(request):
    ...
    # Check if the user liked the movie
    user_liked = Like.objects.filter(user=request.user, movie=movie, is_liked=True).exists()
